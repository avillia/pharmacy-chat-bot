...
from .models import ConversationContext, NewPharmacyLead, Pharmacy


def find_pharmacy_by_phone(
    pharmacies: List[Pharmacy], phone: str
) -> Optional[Pharmacy]:
    """
...
def create_pharmacy_summary(pharmacy: Pharmacy) -> str:
    ...
    if pharmacy.prescriptions:
        top_drugs = sorted(pharmacy.prescriptions, key=lambda x: x.count, reverse=True)[
            :3
        ]
...
def initialize_conversation(
    caller_phone: str, pharmacy: Optional[Pharmacy] = None
) -> ConversationContext:
    """
