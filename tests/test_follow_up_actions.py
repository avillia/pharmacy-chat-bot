...
class TestFollowUpActions:
    ...
    def incomplete_lead(self):
        return NewPharmacyLead(phone="+1-555-INCOMPLETE", name="Incomplete Pharmacy")

    def test_send_email_basic(self, capsys):
        result = send_email("test@example.com", "Test Subject", "Test content here")
    ...
    def test_send_email_custom_sender(self, capsys):
        result = send_email(
            "recipient@example.com", "Custom Subject", "Custom content", "Custom Sender"
        )
    ...
    def test_send_pharmacy_welcome_email_high_volume(
        self, high_volume_pharmacy, capsys
    ):
        ...
        captured = capsys.readouterr()
        assert (
            "As a high-volume pharmacy, you're exactly who we love working with!"
            in captured.out
        )
...
def test_follow_up_actions_integration():
    ...
    callback_id = schedule_callback(
        "+1-555-CALLBACK-TEST", notes="Integration test callback"
    )
