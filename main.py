...
WARNING_NO_OPENAI = (
    "[yellow]⚠️  No OpenAI API key found. Using fallback responses.[/yellow]"
)
...
BOT_GOODBYE = (
    "🤖 [bold green]Bot:[/bold green] Thank you for chatting with us! Have a great day!"
)
...
def execute_follow_up_actions(context: ConversationContext):
    if context.is_returning_customer and context.pharmacy:
        ...
        schedule_callback(
            lead.phone, notes=f"New lead follow-up: {lead.name or 'Unknown pharmacy'}"
        )
...
def chat(
    phone: Annotated[
        str, typer.Option("--phone", "-p", help="Your pharmacy's phone number")
    ] = "+1-555-DEMO-CHAT",
):
    """
    ...
    """
    console.print(
        Panel.fit(WELCOME_PANEL_CONTENT.format(phone=phone), title=WELCOME_PANEL_TITLE)
    )
    ...
    greeting = (
        chatbot.generate_greeting(context)
        if chatbot
        else get_fallback_greeting(context)
    )
    console.print(f"\n{BOT_PREFIX} {greeting}")
...
def demo(
    scenario: Annotated[
        str, typer.Option("--scenario", "-s", help="Demo scenario to run")
    ] = "all",
):
    """
    ...
    """
    console.print(Panel.fit(DEMO_PANEL_CONTENT, title=DEMO_PANEL_TITLE))
...
def test():
    ...
    console.print(
        "  • Demo scenarios: [bold]pytest tests/test_demo_scenarios.py -v[/bold]"
    )
    console.print(
        "  • PromptManager: [bold]pytest tests/test_prompt_manager.py -v[/bold]"
    )
    console.print(
        "  • Follow-up actions: [bold]pytest tests/test_follow_up_actions.py -v[/bold]"
    )
